...
more efficient simply by batching the inputs, calling the network once then 
un-batching them.
...
I speculate that this is due to the concatenation operation being relatively cheaper
though further investigation would be needed.
...
The key to making this trick work is that we evaluate all pairs on the positive
samples, then roll the _outputs_, not the inputs to create the negatively labelled pairs.
...
_Thanks to [D. Lowl](https://bsky.app/profile/d-lowl.bsky.social) for his feedback on an earlier draft of this post_
